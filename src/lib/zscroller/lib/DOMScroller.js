...
DOMScroller.prototype.bindEvents = function bindEvents() {
  ...
  var lockMouse = false;
  var releaseLockTimer = void 0, WheelStopTimer = void 0;
  var mouseScrollYLength = 0, logWheelDelta = 0, maxLogDelta = 0;
  ...
  this.container.addEventListener('mousewheel', this.onMouseWheel = function (e) {
      ...
      if(e.wheelDelta < 0) logWheelDelta = -3 * (Math.log(Math.abs(e.wheelDelta)));
      else if(e.wheelDelta == 0) logWheelDelta = 0;
      else logWheelDelta = 3 * Math.log(e.wheelDelta);
      
      if(Math.abs(logWheelDelta) > maxLogDelta) maxLogDelta = Math.abs(logWheelDelta);

      mouseScrollYLength += logWheelDelta;
      that.scroller.doWheelScroll(mouseScrollYLength, e.timeStamp, e.pageX, e.pageY);
      WheelStopTimer = setTimeout(function() {
	  that.scroller.doTouchEnd(e.timeStamp);
	  mouseScrollYLength = 0;
	  logWheelDelta = 0;
	  maxLogDelta = 0;
	  WheelStopTimer = null;
	  document.removeEventListener('mousewheel', _this2.onMouseWheel, false);
      }, maxLogDelta * 16); 	  
